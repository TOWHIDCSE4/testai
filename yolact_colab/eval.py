...
def distance_measure(box1, box2, img, color):
    distance_color = color
    ...
    x1_b_1, y1_b_1, x2_b_1, y2_b_1 = box1
    x1_b_2, y1_b_2, x2_b_2, y2_b_2 = box2

    # Calculate centroids of the bounding boxes
    center_x1 = (x1_b_1 + x2_b_1) / 2
    center_y1 = (y1_b_1 + y2_b_1) / 2
    center_x2 = (x1_b_2 + x2_b_2) / 2
    center_y2 = (y1_b_2 + y2_b_2) / 2

    # Calculate Euclidean distance between the centroids
    distance_pixels = np.sqrt((center_x2 - center_x1) ** 2 + (center_y2 - center_y1) ** 2)

    # Estimate distance in millimeters (assuming a known pixel-to-mm conversion factor)
    # Adjust the conversion factor according to your specific use case
    pixel_to_mm = 0.7  
    estimated_distance_mm = round(distance_pixels * pixel_to_mm, 2)

    # Draw the line between the centroids of the bounding boxes
    distance_start_point = (int(center_x1), int(center_y1))
    distance_end_point = (int(center_x2), int(center_y2))
    img = cv2.line(img, distance_start_point, distance_end_point, distance_color, 3)

    # Determine the position for the distance text (midpoint of the line)
    mid_x = (distance_start_point[0] + distance_end_point[0]) // 2
    mid_y = (distance_start_point[1] + distance_end_point[1]) // 2
    distance_text_point = (mid_x, mid_y - 10)

    # Draw the distance text
    img = cv2.putText(img, "~" + str(estimated_distance_mm) + " mm", distance_text_point, font, font_scale, font_color, font_thickness, cv2.LINE_AA)
    ...
    # print(x1_b_1, y1_b_1, x2_b_1, y2_b_1)
    # print(x1_b_2, y1_b_2, x2_b_2, y2_b_2)
...
def prep_display(dets_out, img, h, w, undo_transform=True, class_color=False, mask_alpha=0.45, fps_str=''):
    ...
    if args.display_text or args.display_bboxes:
        for j in reversed(range(num_dets_to_consider)):
            ...
            print("-----", j , "-----")
            if j >= 1:
                img_numpy = distance_measure(boxes[j, :], boxes[j-1, :], img_numpy, color = color)
